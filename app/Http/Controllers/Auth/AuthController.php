...
class AuthController extends Controller
{
    ...
    public function completeSISRegistration(Request $request)
    {
        ...
        try {
            // Get or create Alumni role
            $alumniRole = Role::where('name', Role::ALUMNI)->first();
            if (!$alumniRole) {
                throw new \Exception('Alumni role not found in database. Please run database seeders.');
            }
            ...
            // Send password via SMS (Arkesel) instead of email.
            $smsSent = false;
            $smsSendError = null;
            $smsMessage = sprintf(
                'STU Alumni Portal. Temporary password: %s. Login: %s',
                $autoGeneratedPassword,
                url('/login')
            );

            try {
                // Avoid hitting real SMS provider during automated tests.
                if (!app()->environment('testing')) {
                    ArkeselSmsService::send(
                        $alumni->phone,
                        $smsMessage,
                ...
                $smsSendError = $e->getMessage();
                Log::error('SIS registration SMS send failed', [
                    'alumni_id' => $alumni->id,
                    'phone' => $alumni->phone,
                    'error' => $smsSendError,
                ]);
            ...
            return response()->json([
                'success' => true,
                'message' => $smsSendError
                    ? 'Registration completed, but SMS delivery failed. Please contact support for assistance.'
                    : 'Registration completed successfully! A secure password has been generated and sent via SMS to your phone number.',
            ...
            // Log to main Laravel log as well for debugging
            \Log::error('SIS Registration Failed', [
                'error' => $e->getMessage(),
    ...
    public function processManualRegistration(Request $request)
    {
        ...
        try {
            ...
            try {
                ...
                $smsSendError = $e->getMessage();
                Log::error('Manual registration SMS send failed', [
                    'alumni_id' => $alumni->id,
                    'phone' => $alumni->phone,
                    'error' => $smsSendError,
                ]);
